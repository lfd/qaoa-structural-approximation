...
\documentclass[aps,rpx,reprint,amsmath,amssymb,a4paper,noarxiv,twocolumn]{quantumarticle}
...
\end{yquant}
    \coordinate (bl) at ([yshift=1.5\dist]mixer.north west);
    \coordinate (br) at ([yshift=1.5\dist]problem.north east);
    ...
    %\draw[red, dashed] ([xshift=-0.5em]instance_opt.west) -- ++(0.5\textwidth - 0.5\columnsep, 0);

    \begin{pgfonlayer}{background}
        \node[fill=gray!25, draw=none, rounded corners, inner sep=0.5em, fit=(instance_opt.north west) (pp.south east) (meas4.south) (flow_tr)] (outline) {};
    \end{pgfonlayer}

    \coordinate (center) at ($(outline.west |- optimisation.north)!0.5!(outline.east |- optimisation.north)$);
    \node at ([yshift=1.25\dist]center) {\emph{Standard QAOA}};
\end{tikzpicture}
\end{preview}
